...
# ID file load
tmax <- "CR2MET_tmax_v2.5_day_COM_TS_1980_2021.csv" 
tmin <- "CR2MET_tmin_v2.5_day_COM_TS_1980_2021.csv" 

# Open data 
tmin <- rio::import(paste0(data_inp, tmin)) %>% janitor::clean_names()
tmax <- rio::import(paste0(data_inp, tmax)) %>% janitor::clean_names()
...
# Heat groups
hw_data <- hw_data |> 
  mutate(tmax_group = case_when(
    tmax < 15.6 ~ 1, 
    tmax >= 15.6 & tmax < 18.3 ~ 2, 
    tmax >= 18.3 & tmax < 21.1 ~ 3, 
    tmax >= 21.1 & tmax < 23.9 ~ 4, 
    tmax >= 23.9 & tmax < 26.7 ~ 5, 
    tmax >= 26.7 & tmax < 29.4 ~ 6, 
    tmax >= 29.4 & tmax < 32.2 ~ 7, 
    tmax >= 32.2 & tmax < 35 ~ 8,
    tmax >= 35 ~ 9,
    TRUE ~ NA_real_
  )) |> 
  mutate(tmax_group = factor(tmax_group, 
                             levels = 1:9,
